...
export const LoginDesktop: React.FC<LoginDesktopProps> = ({
  ...
  useEffect(() => {
    // @ts-ignore
    if (window !== undefined && !window?.neo4jDesktopApi) {
      setLoading(false);
    ...
    if (window !== undefined) {
      // @ts-ignore
      (window as Neo4jDesktopWindow)?.neo4jDesktopApi
        .getContext()
        .then((context: any) => {
  ...
  const handleSubmit = () => {
    ...
    const { url, host, port, username, password } = (
      graph! as Record<string, any>
    ).connection.configuration.protocols.bolt;
  ...
  const connectToActiveGraph = () => {
    const graphs = projects.reduce(
      (state: any[], project: Record<string, any>) =>
        state.concat(project.graphs),
      []
    );
    const active = graphs.find(
      (graph: Record<string, any>) => graph.status === "ACTIVE"
    );

    if (!active)
      setError(
        new Error(
          "There is no active graph.  Click the `Start` button on a Database in Neoj Desktop and try again."
        )
      );

    const { host, port, username, password } =
      active.connection.configuration.protocols.bolt;
    const scheme = "neo4j";
  ...
  const graphs = project?.graphs && (
    <FormSelect
      ...
      id="graph"
      label="Graph"
      options={project?.graphs?.map((value) => ({
        key: value.id,
        text: value.name,
        value,
      }))}
  ...
  return (
    <div>
      ...
      <FormSelect
        ...
        id="project"
        label="Project"
        options={projects.map((value: Record<string, any>) => ({
          key: value.id,
          text: value.name,
          value,
        }))}
      ...
      <div className={classNames.loginOtherDatabase}>
        <a onClick={() => hide()}>Or connect to another graph</a>
      </div>
    </div>
  );
};
